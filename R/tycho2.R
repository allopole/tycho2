...
#'
#'     Although the Tycho 2.0 database can be querried directly by passing a manually assembled API call URL to \code{read.csv}, as below...
#'
...
#'
#' @param path string (optional). Must be either "query" to perform data queries, or one of the tycho 2.0 database fields to retrieve variable listings.
#' @param params list (optional). A list of query terms in the form \code{list(var1=value1,var2=value2,...)}
#' @param queryterms character vector (optional). Vector of query terms passed as strings in the form \code{c("var1[operator]value1", "var2[operator]value2", ...)}. Dates must be passed this way using the \code{>=} and \code{<=} operators (i.e. \code{queryterms=c("PeriodStartDate>=2000-01-01")})
#' @param apikey string. (required).   Your Project Tycho API key. This can also be passed with \code{params} or \code{queryterms}.
...
tycho2 <- function(path="", params=NULL, queryterms=NULL, apikey=NULL, baseurl="https://www.tycho.pitt.edu/api/"){
  ...
  out <- read.csv(apicall(baseurl=baseurl,path=path,params=p,queryterms=queryterms,apikey=apikey))
  more <- nrow(out)>=5000
  while (more == TRUE) {
